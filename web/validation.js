...
(function () {
    ...
    function chart(mount, cfg) {
        ...
        (cfg.guides || []).forEach(g => {
            ...
            svg.appendChild(el('text', { x: sx(gx[1]) - 4, y: sy(gy[1]) + 24, fill: C.muted,
                                         'font-size': 12, 'text-anchor': 'end',
                                         stroke: C.surface, 'stroke-width': 3,
                                         'paint-order': 'stroke',
                                         'stroke-linejoin': 'round' }, g.label));
        });
    ...
    window.renderValidation = async function renderValidation() {
        ...
        document.getElementById('study-stats').innerHTML =
            ...
            statTile('Temporal order', f3(d.temporal.fitted_order_l2), 'theory: 1 (forward Euler)', 'good') +
        ...
        document.getElementById('cap-temporal').textContent =
            ...
            `${d.temporal.N}×${d.temporal.N} grid, against a reference run with a 256× smaller step.`;
        chart(document.getElementById('chart-temporal'), {
            xLabel: 'Δt', yLabel: 'L₂ error', xLog: true, yLog: true,
            fmt: { x: e2, y: e2 },
            ariaLabel: 'Log-log plot of L2 error against time step, slope one',
            series: [{ name: 'Forward Euler', color: C.s1,
                       points: d.temporal.points.map(p => [p.dt, p.l2]),
                       label: `order ${f3(d.temporal.fitted_order_l2)}` }],
            guides: [{ slope: 1, label: 'slope 1', from: 0, offset: 0.4 }]
        });
        ...
        document.getElementById('study-tables').innerHTML =
            ...
            table(['F', 'Δt', 'steps', 'L₂', 'L∞'],
                d.temporal.points.map(p => [p.F, e2(p.dt), p.steps, e2(p.l2), e2(p.linf)])) +
            '<h4>Stability</h4>' +
